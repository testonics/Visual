...
import in.testonics.omni.image.exception.VisualComparisonException;
import in.testonics.omni.image.exception.VisualNotFoundException;
...
/**
 * Tools for the {@link VisualComparison} object.
 */
public final class VisualComparisonUtil {
     ...
     * @throws VisualComparisonException due to read the image from resources.
     */
    public static BufferedImage readImageFromResources(String path) throws VisualComparisonException {
        File imageFile = new File(path);
        if (imageFile.exists()) {
            try {
                return ImageIO.read(imageFile);
            } catch (IOException e) {
                throw new VisualComparisonException(String.format("Cannot read image from the file, path=%s", path), e);
            }
        } else {
            InputStream inputStream = VisualComparisonUtil.class.getClassLoader().getResourceAsStream(path);
            if (inputStream != null) {
                try {
                ...
                throw new VisualNotFoundException(String.format("Image with path = %s not found", path));
            }
        }
    }
     ...
     * @throws VisualComparisonException due to save image.
     */
    public static void saveImage(File pathFile, BufferedImage image) throws VisualComparisonException {
        ...
        if (!dirExists) {
            throw new VisualComparisonException("Unable to create directory " + dir);
        }
        try {
            ImageIO.write(image, "png", pathFile);
        } catch (IOException e) {
            throw new VisualComparisonException(
